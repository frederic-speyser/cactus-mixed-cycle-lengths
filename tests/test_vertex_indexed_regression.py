"""
test_vertex_indexed_regression.py

Locks mgonal_cactus_series_omega.py -- the VERTEX-indexed engine used
elsewhere in this project (e.g. by test_theorem_a_untabulated_omega.py to
check Theorem 3) -- against silent regressions, by anchoring its output
for four representative Omega against a fixed snapshot of values it
produced earlier.

Note: this is NOT the same indexing convention as the OEIS submission
fiches or the b-files in bfiles/, which are BLOCK-indexed (computed by
mgonal_cactus_series_omega_blocks.py, per the existing OEIS entries for
this family -- A332648, A332649, etc.). The two conventions diverge for
mixed Omega; do not compare the numbers below against the b-files, they
describe a different (though related) count. For a regression anchor on
the actual OEIS-submitted (block-indexed) data, see
test_oeis_data_regression.py.

This file was named test_oeis_sequences_regression.py in earlier versions
of this repository; it was renamed and its description corrected because
the old name and docstring incorrectly suggested it tested the OEIS
fiches themselves.
...
Run: python3 -m pytest test_vertex_indexed_regression.py -v
"""
import sys
import os
...
def test_vertex_indexed_series_unchanged():
...
if __name__ == "__main__":
    test_vertex_indexed_series_unchanged()
    print(f"All {len(EXPECTED) * 2} vertex-indexed sequences reproduced exactly.")
